...
#   tools/transfer-signing-key.sh <key-id> <dest-ssh-host>
#
# Example (Spencer, on nuc-1, sending his key to kiosk):
#   tools/transfer-signing-key.sh inspector@tcos.us kiosk.lab.tcos.us
set -euo pipefail

key_id="${1:?usage: transfer-signing-key.sh <key-id> <dest-ssh-host>}"
dest_host="${2:?usage: transfer-signing-key.sh <key-id> <dest-ssh-host>}"
