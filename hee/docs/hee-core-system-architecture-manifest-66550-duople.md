...
* The Bootstrap Core: Anchor nodes are provisioned as read-only, stateless memory loops. They store the master public GPG key rings, manage the cryptographically signed DNSSEC zone definitions, and broadcast the absolute tcosEpoch 0-second baseline clock pulse (heeEpoch until fleet-ops#419) across the local routing infrastructure.
