...
# SYNOPSIS
    ...
    hee-print [--no-header] [--animate] [--] FILE...
...
# DESCRIPTION
    ...
    Images render as symbols by default, which works inside tmux over ssh
    on any terminal. A terminal with a real graphics protocol can do better:
    set HEE_PRINT_IMAGE_FORMAT to kitty, sixels or iterm (in heerc or the
    environment). Inside tmux that also needs `allow-passthrough on`.
